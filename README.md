...
The test harness (`test-llm.mjs`) makes real API calls using the exact prompts from `package.json` against realistic code samples. It covers 66 scenarios: 33 selection model tests (nested constructs, ordinal branches, multi-function, content-based, line ranges), 12 semantic go-to tests, and 21 router tests (across all 4 routing modes: with/without selection, with/without agent).
